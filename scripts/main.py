...
def create_run_plot_model(name, **kwargs):
    ...
    # swt = cam.build_model(pars, sea_level_rise=1, rise_type="linear", rise_length=365*100)
    # cam.run_model(swt)
    # concentration, head, qx, qy, qz = cam.extract_results(name)
    results.plot_sea_level_rise_results(name, rise_length=365*100)
...
def main():
    ...
    create_run_plot_model("test_slr", x_w=100, z_w=-2, h_w=0, h_b=0.5, nlay=20, drain_conductance=100000, Lz=6)
